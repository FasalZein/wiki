...
export const USAGE_REGISTRY: Record<string, UsageEntry> = {
  ...
  doc: {
    ...
    example: 'wiki doc recategorize NOTE-0001 --project myproj --category architecture',
    subverbs: {
      retitle: {
        summary: "Rename an artifact's title and re-slug its filename (stays in its current folder).",
        usage: "wiki doc retitle <id> --project <name> --title <new title> [--json]",
        flags: { "--project": "project name (required)", "--title": "new title (required)", "--json": "emit {id,path} to stdout" },
        example: 'wiki doc retitle NOTE-0001 --project myproj --title "Clearer title"',
      },
      recategorize: {
        summary: "Move an artifact into another bucket (only applies to a branch section with sub-buckets).",
        usage: "wiki doc recategorize <id> --project <name> --category <bucket> [--json]",
        flags: { "--project": "project name (required)", "--category": "a declared bucket of the artifact's section", "--json": "emit {id,path} to stdout" },
  ...
  "next-id": {
    ...
    usage: "wiki next-id <prd|slice|decision|architecture|research|runbooks|specs|notes|legacy|handoff> --project <name>",
