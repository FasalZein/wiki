...
export async function handleGreen(args: string[]): Promise<CliResult> {
  ...
  try {
    ...
    const beforeRun = decideFromArtifact(artifact, "green");
    if (!beforeRun.ok) {
    ...
    const logPath = join(config.repo, ".wiki", "state", "slices", `${id}-green.log`);
    const exitCode = await captureTestRun(config.repo, config.test_command, logPath);
    const afterRun = decideFromArtifact(artifact, "green", exitCode);
    if (!afterRun.ok) {
      console.error(afterRun.reason);
      return { code: afterRun.exitCode };
    }

    await setFields({ type: "slice", vaultRoot, project, id, fields: { status: "green", green_log_ref: logPath } });
    console.log(logPath);
    console.error(`green captured at ${logPath}`);
    await writePhaseDocToStderr(config.repo, "close", phaseDocOptions(parsed));
    return { code: 0 };
  } catch (error) {
...
function decideFromArtifact(artifact: Artifact, verb: "green", capturedExitCode?: number) {
