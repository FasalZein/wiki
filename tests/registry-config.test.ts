...
describe("config-driven kind registry (wiki.json)", () => {
  test("kinds are loaded from wiki.json with prefix/folder/dedup", () => {
    ...
    expect(ARTIFACTS.prd).toEqual({ prefix: "PRD", folder: "prds", dedup: true, skill: "to-prd" });
    expect(ARTIFACTS.doc).toEqual({ prefix: "DOC", folder: "docs", dedup: true }); // no authoring skill
