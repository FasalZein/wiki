...
function renderedBody(template: string, values: Record<string, unknown>): string {
  return matter(renderArtifact(template, values)).content;
}
...
describe("renderArtifact each-blocks", () => {
  test("expands an each-block over list values with {{this}}", () => {
    const out = renderedBody(tpl("{{#each acceptance}}- [ ] {{this}}\n{{/each}}"), {
  ...
  test("renders the else branch when the field is missing", () => {
    const out = renderedBody(tpl("{{#each tags}}`{{this}}` {{else}}_none_{{/each}}"), {});
  ...
  test("expands wiki links for link_list values", () => {
    const out = renderedBody(tpl("{{#each blocked_by}}- [[{{this}}]]\n{{else}}None — can start immediately.\n{{/each}}"), {
